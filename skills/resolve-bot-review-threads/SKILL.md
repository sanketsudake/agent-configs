...
## 6. GraphQL Mutation Templates (Copy-Paste Ready)

```graphql
# List unresolved threads (query)
query($owner:String!, $repo:String!, $pr:Int!) {
  repository(owner:$owner, name:$repo) {
    pullRequest(number:$pr) {
      reviewThreads(first:100) {
...
# Reply to a thread (mutation) — note: pullRequestReviewThreadId, not threadId
...
# Discover bot ID from an existing review (query)
{
  repository(owner:"OWNER", name:"REPO") {
...
# Discover bot ID via suggestedActors (alternative)
{
  repository(owner:"OWNER", name:"REPO") {
    suggestedActors(capabilities:[CAN_BE_ASSIGNED], first:30) {
      nodes {
